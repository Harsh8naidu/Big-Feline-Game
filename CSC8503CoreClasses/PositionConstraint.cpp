...
void PositionConstraint::UpdateConstraint(float dt)	{
	...
	if (abs(offset) > 0.0f) {
		...
		if (constraintMass > 0.0f) {
			// how much of the relative velocity is in the direction of the constraint
			...
			physA->ApplyLinearImpulse(aImpulse); // apply impulse to object A
			physB->ApplyLinearImpulse(bImpulse); // apply impulse to object B
		}
	}
}
